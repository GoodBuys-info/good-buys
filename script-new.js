document.addEventListener("DOMContentLoaded", function () {
    fetch('labelsdata.json')
        ...
        .then(data => {
            ...
            // Display the sorted ecolabels
            sortedData.forEach(label => {
                const labelElement = document.createElement('div');
                labelElement.textContent = label.name;
                document.getElementById('image-container').appendChild(labelElement);
            });

            const footerImages = sortedData; // Use sortedData for footerImages
